...
Краткая инструкция: переменные окружения, CORS для `*.vercel.app` и кастомного домена — в **[VERCEL.md](./VERCEL.md)**. Версия Node для сборки: **≥ 20.9** (`engines` в `package.json`).
