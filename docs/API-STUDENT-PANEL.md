...
Аналог квиза по **уроку** (вместо устаревшего `GET /app/modules/:moduleId/quiz`).

**Query `lang` (опционально):** `ru` — по умолчанию, если параметр не передан; `kk` — казахский; регистр не важен. Иное значение — **`400`**. Для текстов квиза (`title`, `text` вопросов, `text` вариантов) бэкенд отдаёт строки на выбранном языке с **пофаллбеком на русский по каждому полю**, если перевода нет.

**Ответ `200`:** объект квиза; в корне может быть **`language`**: `"ru"` | `"kk"` — запрошенный `lang`. Поле **`maxAttempts`** — **эффективный** лимит попыток для текущего ученика. Опционально бэкенд может отдать **`maxAttemptsSource`** — откуда взят лимит.

**Сдача:** `POST …/attempt` и `POST …/submit` без параметра языка; для вопросов типа `text` верный ответ принимается по русскому или казахскому эталону (без учёта регистра), если казахский задан в админке. Рекомендация UI: передавать в GET тот же `lang`, что и локаль интерфейса ученика.

**Приоритет эффективного лимита (сверху вниз):**

1. Переопределение ученик + тест (`user_quiz_max_attempt_overrides` / аналог на бэкенде).
2. Лимит в записи доступа к курсу (`course_accesses.max_quiz_attempts`).
3. Дефолт курса (`courses.default_max_quiz_attempts`).
4. Поле теста (`quizzes.max_attempts`).
...
`POST /app/quizzes/:quizId/attempt` использует **тот же** эффективный лимит попыток, что и `GET …/quiz`: при исчерпании лимита — **`400`**.
